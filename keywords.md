# SEO Keywords Map

This document tracks the target keywords and metadata for each page to ensure we are mindful of our SEO strategy.

## Guidelines

- **Title**: Should be under 60 characters and include the primary keyword.
- **Description**: Should be under 160 characters and include primary and secondary keywords, encouraging click-through.
- **Keywords**: A list of 3-5 focus keywords for the page.

---

## English Pages

### Home (`/`)

- **Five-second summary**: Portfolio home for Alexis Gomel, highlighting physics and data science.
- **Title**: Alexis Gomel | Physicist & Data Scientist Portfolio
- **Description**: _[Imported from SITE_DESCRIPTION constant]_
- **Target Keywords**:
  - Alexis Gomel
  - Data Scientist Portfolio
  - Physicist Portfolio
  - Data Analysis

### Projects Overview (`/projects/`)

- **Five-second summary**: Gallery of all data science, physics, and app projects.
- **Title**: Data Analysis, Dashboards & Apps | Alexis Gomel
- **Description**: Explore my Data Science and Physics projects, including nonlinear dynamics, web scraping, and data visualization tools.
- **Target Keywords**:
  - Data Analysis & Dashboards
  - App Development
  - Data Visualization Development
  - Dashboard Tools

### The Seville Palette (`/projects/sevilla/`)

- **Five-second summary**: A custom color palette inspired by Seville.
- **Title**: The Seville Palette | Data Visualization
- **Description**: A categorical color palette inspired by the history and landscapes of Seville, visualized as a network of accessible contrasts.
- **Target Keywords**:
  - Seville Color Palette
  - Accessible Color Splits
  - Data Visualization Colors
  - Network Visualization

### Photography (`/photography/`)

- **Five-second summary**: Photography portfolio section.
- **Title**: Photography | Alexis Gomel
- **Description**: _[To be filled]_
- **Target Keywords**:
  - Street Photography
  - [Location] Photography

### Online CV (`/cvonline/`)

- **Five-second summary**: Digital version of the curriculum vitae.
- **Title**: Online CV | Alexis Gomel
- **Description**: _[To be filled]_
- **Target Keywords**:
  - Alexis Gomel CV
  - Data Scientist Resume
  - Physicist Resume

### Playlists (`/playlists/`)

- **Five-second summary**: Music playlists and sharing.
- **Title**: Playlists | Alexis Gomel
- **Description**: _[To be filled]_
- **Target Keywords**:
  - Swing Music Playlists
  - Blues Music Playlists

### Contact (`/contact/`)

- **Five-second summary**: Contact form and information.
- **Title**: Contact | Alexis Gomel
- **Description**: _[To be filled]_
- **Target Keywords**:
  - Contact Alexis Gomel

### SHARE Calculator (`/sac/`)

- **Five-second summary**: Shared Accommodation Rent Calculator tool.
- **Title**: SHARE Calculator
- **Description**: Simple calculator to approximate the share of rent each tenant should pay based on room size.
- **Target Keywords**:
  - Rent Split Calculator
  - Roommate Rent Split
  - Square Footage Rent Calculator
  - Shared Apartment Area Calculator
  - Fair Rent Calculator

---

## Spanish Pages (`/es/`)

_> Complete this section mirroring the English pages but with Spanish keywords._

### Inicio (`/es/`)

- **Title**: _[To be filled]_
- **Description**: _[To be filled]_
- **Target Keywords**:
  - Alexis Gomel Portfolio Español
  - Científico de Datos

### Calculadora SHARE (`/es/sac/`)

- **Title**: Calculadora de Alquiler Justo por Metros Cuadrados | SHARE
- **Description**: ¿Necesitas dividir el alquiler con compañeros de piso? La Calculadora SHARE te ayuda a estimar un reparto justo basado en los metros cuadrados y las zonas comunes.
- **Target Keywords**:
  - Calculadora de Alquiler
  - Reparto de Alquiler Compañeros de Piso
  - Alquiler por Metros Cuadrados
  - Calculadora de Pisos Compartidos

### Proyectos (`/es/projects/`)

- **Title**: Ciencia de Datos, Dashboards y Apps | Alexis Gomel
- **Description**: Explora mis proyectos de Ciencia de Datos, incluyendo Análisis de Datos y Dashboards, Desarrollo de Apps y Herramientas de Dashboard. Con Visualización de Datos, dinámica no lineal y web scraping.
- **Target Keywords**:
  - Ciencia de Datos
  - Análisis de Datos y Dashboards
  - Desarrollo de Apps
  - Desarrollo de Visualización de Datos
  - Herramientas de Dashboard
  - Cuadro de Mando

### Sevilla (`/es/projects/sevilla/`)

- **Title**: Paleta de Colores Accesible de Sevilla y Visualización | Alexis Gomel
- **Description**: Explora 'La Paleta de Sevilla', un esquema de color accesible inspirado en la historia de Sevilla. Incluye visualizaciones interactivas, análisis de contraste WCAG y simulaciones de daltonismo.
- **Target Keywords**:
  - Paleta de Colores de Sevilla
  - Paleta de Colores Accesible
  - Análisis de Contraste WCAG
  - Visualización de Datos de Color
  - Simulador de Daltonismo Sevilla

### Analítica Web (`/es/projects/webanalytics/`)

- **Title**: Dashboard GA4, BigQuery y Looker Studio | Alexis Gomel
- **Description**: Demostración en vivo de un stack de analítica web personalizado usando GA4, BigQuery y Looker Studio. Visualiza tráfico y datos de usuario en tiempo real.
- **Target Keywords**:
  - Dashboard Analítica Web
  - Proyecto GA4 y BigQuery
  - Portafolio Looker Studio
  - Stack Analítica Personalizado
  - Visualización de Datos en Tiempo Real
